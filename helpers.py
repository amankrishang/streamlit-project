...
def medal_tally_calculator(df):
    medal_counts_winter = df.groupby(["NOC", "Medal"]).size().reset_index(name="count")
    medal_pivot = medal_counts_winter.pivot(index="NOC", columns="Medal", values="count").fillna(0)
    medal_pivot_winter = medal_pivot.astype(int)
